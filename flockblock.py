...
def get_gps():
    """
    Poll GPS receiver and return latitude, longitude tuple.
    Turn on the 'valid fix LED' if a 2D or 3D fix is obtained.

    :return: Tuple containing current latitude, current longitude.
    """
...
while True:
    ...
    time.sleep(0.5)
    clat, clon = get_gps() # Get GPS location
    ...
    if clat is not None: # Valid 2D or 3D fix
        ...
        while checkresult: # Inside the geofence boundary
            ...
            time.sleep(0.5)
            clat, clon = get_gps() # Get GPS location
            checkresult = scanlist(lllist, clat, clon, flockradius) # Do not proceed outside loop until we are NOT inside the geofence boundary
